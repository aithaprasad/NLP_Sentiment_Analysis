...
def run_from_scratch(hyper_tune = False, with_test = False):
    ...
    if hyper_tune is True:
        ...
        pos_ngrams = ngrams_gen(pos, best_params['ngram'])
        neg_ngrams = ngrams_gen(neg, best_params['ngram'])

        counts_full = get_counts(pos_ngrams, neg_ngrams)

        counts = min_count_remove(counts_full, best_params['min_count'])

        priors = get_priors(counts, pos, neg, best_params['alpha'])

        scores = predict(test, priors, offsets, best_params['ngram'])
